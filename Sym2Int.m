...
Block[{result},result={};
AppendTo[result,Row[{Style["XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX ",{GrayLevel[0.5]}],Hyperlink[Mouseover[Style["Sym2Int",{GrayLevel[0.5]}],Style["Sym2Int",{Darker[Blue,0.5],Bold}]],"http://renatofonseca.net/sym2int.php"],Style[" XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX",{GrayLevel[0.5]}]}]];
AppendTo[result,Row[{Style["Version: 2.2 (07/July/2022); Author: Renato Fonseca.",{GrayLevel[0.5]}]}]];
...
AppendTo[result,Row[{Hyperlink[Mouseover[Style["This",{GrayLevel[0.5],Bold,Underlined}],Style["This",{Darker[Blue,0.5],Bold,Underlined}]],"http://renatofonseca.net/sym2int.php"],Style[" webpage explains how to use it.",{GrayLevel[0.5]}]}]];
AppendTo[result,Style["XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX",{GrayLevel[0.5]}]];
...
If[!$Sym2IntUseEOM,
Print[Style["NOTE:",{Bold,Underlined,FontFamily->"Consolas",Black}],Style["The flag $Sym2IntUseEOM is set to False, which mean that equations of motion are not being used to remove redundant operators. As a consequence Sym2Int will not distinguish the fields F1, F2, ... (the left-handed part of the field strenght tensors, transforming as (1,0) under the Lorentz group) from their conjugates F1*, F2*, ... (transforming as (0,1) under the Lorentz group). Furthermore, all operators with F1(*), F2(*), ... will be marked as being real. The reason: without the equations of motion, the Bianchi identities can be used to remove operators with Fi or Fi*, turning the counting ambiguous." ,{Bold,FontFamily->"Consolas",Black}]];
];
];
...
fieldsAUX[model]^=ModifyModelFieldsToIncludeDerivatives[model,uniqueFields,1+Length[fields[model]],massDimensions[[2;;-1]],reps\[UnderBracket]discreteCharges];
...
ModifyModelFieldsToIncludeDerivatives[model_,listOfFieldsAppearing_,derIdx_,massDimensions_,reps\[UnderBracket]discreteCharges_]:=Module[{useEOMQ,n,counter,result,fullName,fullNameR,gaugeQN,lorentzQN,realQ,nF,nameOfTheFields,reps\[UnderBracket]gaugeIn,reps\[UnderBracket]gauge,reps\[UnderBracket]lorentzIn,reps\[UnderBracket]lorentz,reps\[UnderBracket]RI,reps\[UnderBracket]copies,rootName,rootLorentzQN,FmunuQN,massD,discreteQN,grassmann,derivative, aux,derivativeReps,isFmunuQ,realFmunuReps,complexFmunuReps},

useEOMQ=$Sym2IntUseEOM;
...
rootName=If[el[[1]]<=n,nameOfTheFields[[el[[1]]]],"F"<>ToString[el[[1]]-n-1]<>If[$Sym2IntUseEOM,"","(*)"]];
...
(* If EOM are not used, then there are move Lorentz irreps to consider *)
If[useEOMQ,
(* ++++++++++++++++ Use EOMs ++++++++++++++++ *)
...
(* In this specific case (el[[2]]>0 && !useEOMQ && isFmunuQ) add here already the fields *)
Do[
counter++;
Sow[{counter,el,fullName,gaugeQN,lQN,"C",1,massD,discreteQN,grassmann}];
,{lQN,complexFmunuReps}];


(* Check in the rest of the code: even though these fields are marked as real ("R"), does de code still try to conjugate them because of the information in position #2 (el)? *)
Do[
counter++;
Sow[{counter,el,fullName,gaugeQN,lQN,"R",1,massD,discreteQN,grassmann}];
,{lQN,realFmunuReps}];
...
,
(* The case el[[2]]>0 && !useEOMQ && isFmunuQ was already taken care of before *)
If[!isFmunuQ,
Do[
counter++;
Sow[{counter,el,fullName,gaugeQN,lQN[[1]],realQ,lQN[[2]]nF,massD,discreteQN,grassmann}];
...
(* If EOMs are not used, assume that all operator classes are real, so that the counting of operators and terms is for real ones *)
If[!$Sym2IntUseEOM,
startFPos=Length[fields[model]]+2;
FmunuPos=Range[startFPos,startFPos-1+Length[gaugeGroup[model]]];
...
fieldNames=Join[{"\[ScriptCapitalD]"},fields[model][[All,1]],Table["F"<>ToString[i]<>If[$Sym2IntUseEOM,"","(*)"],{i,0,Length[gaugeGroup[model]]}]];
